...
import type { AreaGuide } from '../data/zoneGuide';

type Item = { place: string; desc?: string };

const Row = ({ label, items }: { label: string; items: Item[] }) => (
  <div>
...
/** 구역 카드 — 대표 사진은 zoneGuide.photo(있으면) 우선, 없으면 대표 명소로 위키 사진 조회. */
function ZoneCard({ g, isOpen, onToggle }: { g: AreaGuide; isOpen: boolean; onToggle: () => void }) {
  const wiki = usePlacePhoto(g.photo ? undefined : g.mustSee[0]?.place);
  const photo = g.photo || wiki;

  return (
    <div className="overflow-hidden rounded-xl border border-white/5 bg-moose-dusk/70">
      <button onClick={onToggle} className="block w-full text-left">
        {photo ? (
          <div className="relative h-28 w-full">
            ...
            <span className="absolute right-1.5 top-1.5 rounded bg-black/45 px-1 py-0.5 text-[8px] text-white/70">
              {g.photo ? g.credit : '위키피디아'}
            </span>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="edge min-h-0 flex-1 space-y-1.5 overflow-y-auto py-3">
        ...
        {guides.map((g) => (
          <ZoneCard key={g.id} g={g} isOpen={open === g.id} onToggle={() => setOpen(open === g.id ? '' : g.id)} />
        ))}
      </div>
    </div>
  );
